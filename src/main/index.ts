...
import { applyWindowIconToTitleBar, getWindowIconPath } from './app-icon'
...
import { resolveBundledSidecarExecutable, resolvePythonInterpreter } from './python-resolve'
...
function loadEnvFromDotenv(): void {
  mergeEnvFromFile(join(process.cwd(), '.env'))
}
...
/**
 * 개발 시 터미널에 뜨는 `gpu_disk_cache` / `cache_util_win` (0x5) 는 Chromium이
 * 캐시 폴더에 쓰지 못할 때 나는 **앱 비즈니스 로그가 아님**. GPU 셰이더 디스크 캐시를 끄면
 * 대부분 사라지거나 줄어듦(배포본은 기본 캐시 유지).
 */
if (!app.isPackaged) {
...
/** `resources/bin/main.exe` 등 — PyInstaller 사이드카만 쓰고 pip 제외할 때 */
function usesBundledSidecar(): boolean {
  return Boolean(app.isPackaged && resolveBundledSidecarExecutable(process.resourcesPath))
}

/**
 * - 개발 / 번들 없음: 시스템(또는 `PYTHON_PATH`) Python + `-u main.py`
 * - 설치 본(`app.isPackaged`) + `resources/bin/main.exe` 존재: 해당 exe만 (스크립트 인자 없음)
 */
function resolveSidecarCommandAndArgs(): { command: string; args: string[] } {
  if (app.isPackaged) {
    const bundled = resolveBundledSidecarExecutable(process.resourcesPath)
    if (bundled) {
      return { command: bundled, args: [] }
    }
  }
  const script = getSidecarScriptPath()
  return { command: resolvePythonInterpreter(), args: ['-u', script] }
}

function assertSidecarSpawnPaths(spec: { command: string; args: string[] }): void {
  if (!existsSync(spec.command)) {
    throw new Error(`Sidecar 실행 파일이 없습니다: ${spec.command}`)
  }
  if (spec.args.length > 0) {
    const scriptPath = spec.args[spec.args.length - 1]
    if (scriptPath && !existsSync(scriptPath)) {
      throw new Error(`Sidecar 스크립트가 없습니다: ${scriptPath}`)
    }
  }
}
...
function ensureSidecar(): PythonSidecar {
  const spec = resolveSidecarCommandAndArgs()
  assertSidecarSpawnPaths(spec)
  if (!sidecar) {
...
function createWindow(): void {
  /** Windows는 문자열 경로(특히 .ico)가 제목줄 아이콘에 더 잘 반영되는 경우가 많음 */
  ...
  mainWindow.on('ready-to-show', () => {
    if (mainWindow) applyWindowIconToTitleBar(mainWindow)
    mainWindow?.show()
  })
...
app.whenReady().then(() => {
  /** 설치본은 cwd에 개발용 .env가 없음 → %AppData% 등 userData의 .env에서 GitHub PAT 등 로드 */
  ...
  ipcMain.handle('gpu:status', async () => {
    ...
    return {
      ...
      urlConfigured,
      localCandidate,
      dllDir: getGpuRuntimeDir()
    }
  })

  ipcMain.handle('gpu:install', async (event) => {
    if (gpuInstallLocked) throw new Error('GPU 런타임 설치가 이미 진행 중입니다.')
    gpuInstallLocked = true
    const win = BrowserWindow.fromWebContents(event.sender) ?? mainWindow
    const sendGpuProgress = (pct: number, stage: string) => {
      if (win && !win.isDestroyed()) {
        const v = Math.round(Math.max(0, Math.min(100, pct)) * 100) / 100
        win.webContents.send('gpu:install-progress', { pct: v, stage })
      }
    }
    try {
      ...
      logMain(
        'gpu:install',
        '시작',
        `zipUrl=${zipUrl} dllDir=${getGpuRuntimeDir()} localCandidates=${getLocalGpuDllCandidates().join(' | ')}`
      )
      const result = await installGpuRuntime({
        localCandidates: getLocalGpuDllCandidates(),
        zipUrl,
        onProgress: sendGpuProgress
      })
      logMain('gpu:install', '완료', result)
      sidecar?.stop()
      sidecar = null
      return { ok: true as const, source: result.source, dir: result.dir }
    } catch (e) {
      logMain('gpu:install', '실패', e)
      throw e
    } finally {
  ...
  ipcMain.on('video-drop-path', (event, raw: unknown) => {
    void (async () => {
      ...
      let sc: PythonSidecar | null = null
      try {
        sc = ensureSidecar()
        ...
        logMain('transcribe', 'video-drop-path 실패', message)
