...
export function resolvePythonInterpreter(): string {
  ...
  /** spawn으로 경로를 못 얻을 때(일부 환경에서 Electron 자식 PATH 제한) 흔한 설치 위치를 직접 탐색 */
  if (win) {
    for (const dir of collectWindowsPythonPathPrefixes()) {
...
/**
 * 설치 본문(`app.isPackaged`) 기준 `process.resourcesPath/bin` 아래에 두는
 * PyInstaller 등 고정 사이드카 실행 파일.
 *
 * - Windows: `bin/main.exe`
 * - macOS / Linux: `bin/main` (실행 비트)
 */
export function resolveBundledSidecarExecutable(resourcesPath: string): string | null {
  const candidates =
    process.platform === 'win32'
      ? [join(resourcesPath, 'bin', 'main.exe')]
      : [join(resourcesPath, 'bin', 'main'), join(resourcesPath, 'bin', 'main.exe')]
